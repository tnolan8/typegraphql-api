...
async function fetchSha256(url) {
    ...
    const [zippedSha256, sha256] = [
        (await node_fetch_1.default(`${url}.sha256`).then(res => res.text())).split(/\s+/)[0],
        (await node_fetch_1.default(`${url.slice(0, url.length - 3)}.sha256`).then(res => res.text())).split(/\s+/)[0],
    ];
...
async function downloadZip(url, target, progressCb) {
    ...
    const result = await p_retry_1.default(async () => {
        try {
            const resp = await node_fetch_1.default(url, { compress: false, agent: getProxyAgent_1.getProxyAgent(url) });
            if (resp.status !== 200) {
            ...
            return await new Promise(async (resolve, reject) => {
                ...
                const zippedHashPromise = hasha_1.default.fromStream(resp.body, { algorithm: 'sha256' });
